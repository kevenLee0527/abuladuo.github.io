<template>
  <div class="task">
    <div class="header-name">任务管理 / 申述审核任务</div>
    <div class="main">
      ...
      <thead>
        <th :style="{width:'15%'}"><span>投放主题</span> </th>
        <th :style="{width:'15%'}"><span>标题/账户/分类/类型</span></th>
        <th :style="{width:'15%'}"><span>编号/时间/佣金/追加</span></th>
         <th :style="{width:'15%'}"><span>订单状态</span></th>
        <th :style="{width:'20%'}"><span>查看任务详情</span></th>
        <th :style="{width:'20%'}"><span>操作</span></th>
      </thead>
      <tbody>
        ...
        <tr v-for="(item,index) in taskList" :key="index" >
          <td>
           <img class="userImg" :src="item.task_thumb" />
            <div class="item nobor" :style="{borderTop:'1px solid #e6e8ea'}"></div>
          </td>
          <td>
          ...
          <td :style="{textAlign:'center'}">
            <a-button @click="onConfirm(item.task_id,2)" type="primary">审核通过</a-button>
            &nbsp;&nbsp;
            <a-button @click="onConfirm(item.task_id,-1)" type="primary">审核不通过</a-button>
          </td>
        </tr>
      </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    onConfirm(task_id,status){
      ...
      let token = sessionStorage.getItem("token");
      this.$http.post('admin/Task/passStatus',data,{
        headers:{token}
      }).then(res=>{
        console.log(res)
        if(res.data.code==200){
