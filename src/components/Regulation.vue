<template>
  <div class="task">
    <div class="header-name">内容管理 / 相关规则</div>
    <div class="main">
        <div class="task-search-bar">
        标题搜索:
        <a-input :style="{width:'400px',display:'iline-block'}" />
        <div :style="{float:'right'}">
          <a-button :style="{marginLeft:'20px'}" type="primary" @click="showModal">添加规则</a-button>
        </div>
    ...
    <a-modal :style="{top:'20px'}" width="900px" title="添加规则" v-model="visible" @ok="handleOk" okText="确定" cancelText="取消">
      <a-row>
        <a-col :span="3">
