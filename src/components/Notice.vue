<template>
  <div class="task">
    <div class="header-name">内容管理 / 网站公告</div>
    <div class="main">
        <div class="task-search-bar">
        公告搜索:
        <a-input :style="{width:'400px',display:'iline-block'}" />
        <div :style="{float:'right'}">
          <a-button :style="{marginLeft:'20px'}" type="primary" @click="showModal">公告发布</a-button>
        </div>
        <table border="1" cellspacing="0">
      <thead>
        <th :style="{width:'5%'}"><span>ID</span> </th>
        <th><span>公告名称</span></th>
    ...
    <a-modal :style="{top:'20px'}" width="900px" title="公告发布" v-model="visible" @ok="handleOk" okText="确定" cancelText="取消">
      <a-row>
        <a-col :span="3">
